...
class AdminController extends Controller
{
    ...
    // private function viewAdmin($filter = null)
    // {
    //     try {

    //         $data = ['admins.id', 'admins.uid', 'role', 'assignDate', 'retireDate',  'remarks', 'isActive', 'isAuth', 'modifiedDate', 'user_infos.name', 'admin_titles.title',];

    //         if ($filter == 'all') {

    //             $results = DB::table('admins')
    //                 ->join('user_infos', 'admins.id', '=', 'user_infos.uid')
    //                 ->join('admin_titles', 'admins.id', '=', 'admin_titles.uid')
    //                 ->latest('modifiedDate')->limit(50)->get($data)->toJson(JSON_PRETTY_PRINT);
    ...
    //         date_default_timezone_set('Asia/Dhaka');
    //         error_log("Error from viewAdmin@AdminController | " . date('d M Y H:i:s', time()) . " | " . $except->getMessage());
